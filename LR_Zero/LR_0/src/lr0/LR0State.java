...
public class LR0State {
    ...
    public LR0State(Grammar grammar, HashSet<LR0Item> coreItems) {
        items = new LinkedHashSet<LR0Item>(coreItems);
        transition = new HashMap<String, LR0State>();
        closure(grammar);
    }
    ...
    private void closure(Grammar grammar) {
        ...
        do {
            changeFlag = false;
            HashSet<LR0Item> temp = new HashSet<LR0Item>();
    ...
    private HashSet<LR0Item> createLR0Item(HashSet<Rule> rules) {
        HashSet<LR0Item> results = new HashSet<LR0Item>();
        for (Rule rule : rules) {
